...
export class PartsInwardComponent {
  ...
  getAllOrders() {
    const data: any = {};
    this.orderService.getSupplyOrders().subscribe((res: any) => {
      res.map((p: any) => {
        if (p.status === 'Pending') {
          p.partsData.map((o: any) => {
            data[o.partNo] = p.supplyOrderNumber;
          })
        }
      })
    ...
    this.transactionService.getTransaction(transactionNumber).subscribe((res: any) => {
      if (res) {
        ...
        this.partInwardForm = new FormGroup({
          ...
          purchaseOrderNo: new FormControl(res.purchaseOrderNo),
          supplierName: new FormControl('', Validators.required),
  ...
  saveTransaction() {
    ...
    if (this.partInwardForm.value.supplierName.accountName && this.data.length > 0 && this.partInwardForm.value.totalNetAmount) {
      const payload = {
        ...
        purchaseOrderNo: this.partInwardForm.value.purchaseOrderNo,
